    ...
    for d_idx = 1:length(d)
        ...
        for sp = 1:length(sp_value)
            ...
            try
                ...
                % 2000行目の1〜20列の中央値を計算
                median_2000_row = median(data(2000, 1:run));
                
                % 各行の1〜20列の値に対して中央値との差を計算
                diff_to_median = abs(data(2000, 1:run) - median_2000_row); 
            ...
            filename = sprintf('%s_mat/%s_run%d_f%d_d%d_sp%.2f.mat',surrogate, surrogate, closest_row, f_v, d_value, sp_v);
...
%                         writematrix(filled_data,filename1);
        ...
        nos_data = nosdiversity(f_v,d_value,run);
