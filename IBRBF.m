...
% knn = 4 * dim;  %training size for each surrogate model 
...
while fe < maxfe 
    ...
    % Model management:  The ¦Ë highest ranked individuals  are
    ...
    for i=1:pop_size
        ...
        srgtOPTKRG  = srgtsRBFSetOptions(train.x, train.y);
        RBFModel = srgtsRBFFit(srgtOPTKRG);
        offspring_predict_y(i) = srgtsRBFEvaluate(offspring(:, i)', RBFModel);
    end
