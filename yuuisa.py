...
for file in os.listdir(directory):
    if surrogate in file:
        ...
        if match_sp and match_f_d:
            sp = float(match_sp.group(1))
...
for _, row in unique_f_d.iterrows():
    ...
    # 精度のリストを取得してソート
    sp_values = sorted(subset_df['sp'].unique())
    ...
    for i in range(len(sp_values) - 1):
        for j in range(i + 1, len(sp_values)):
            ...
            tukey = pairwise_tukeyhsd(endog=pd.concat([data_sp1, data_sp2]),
                                      groups=['sp{:.2f}'.format(sp_1)] * len(data_sp1) + ['sp{:.2f}'.format(sp_2)] * len(data_sp2),
                                      alpha=0.05)
